...
DOMAIN_SLOT_TEMPLATES = {
    ...
    "restaurant": OrderedDict(
        [
            ...
            (
                ...
                lambda x, either: f"with {either(f'an {x}' if any([x.startswith(prefix) for prefix in ['a', 'e', 'o', 'u', 'i']]) else f'a {x}')} price",
            ),
    ...
    "attraction": OrderedDict(
        [
            (
                "attraction-type",
                lambda x, either: f"which is {either(f'an {x}' if any([x.startswith(prefix) for prefix in ['a', 'e', 'o', 'u', 'i']]) else f'a {x}')}",
            ),
...
def get_first_sentence(ds: Dict, domain: str, either: callable, except_keys: Set[str], idx: int, wo_para: bool):
    ...
    slot_phrases = {
        _slot_name: f(ds[_slot_name], either)
    ...
    res = " ".join(
        ...
        + [_phrase for _slot_name, _phrase in slot_phrases.items() if _slot_name not in except_keys]
    )

    # example: ["has internet", "has no parking"]
    rest_phrases = [_phrase for _slot_name, _phrase in slot_phrases.items() if _slot_name in except_keys]
    if rest_phrases:
...
def train_state_to_sum(ds: dict, either: callable, is_one_sentence: bool, idx: int, wo_para: bool) -> str:
    ...
    first_sentence = get_first_sentence(
        ds, domain="train", either=either, except_keys={"train-arriveby", "train-leaveat"}, idx=idx, wo_para=wo_para
    )

    second_sentence = get_dontcare_sentence(
        ds, domain="train", either=either, is_one_sentence=is_one_sentence, wo_para=wo_para
    )
...
def taxi_state_to_sum(ds: Dict, either: callable, is_one_sentence: bool, idx: int, wo_para: bool) -> str:
    ...
    """
    first_sentence = get_first_sentence(
        ds, domain="taxi", either=either, except_keys={"taxi-arriveby", "taxi-leaveat"}, idx=idx, wo_para=wo_para
    )

    second_sentence = get_dontcare_sentence(
        ds, domain="taxi", either=either, is_one_sentence=is_one_sentence, wo_para=wo_para
    )
    res = first_sentence + second_sentence + "."
...
def restaurant_state_to_sum(ds: Dict, either: callable, is_one_sentence: bool, idx: int, wo_para: bool) -> str:
    ...
    """
    first_sentence = get_first_sentence(
        ds=ds, domain="restaurant", either=either, except_keys={"restaurant-food"}, idx=idx, wo_para=wo_para
    )

    second_sentence = get_dontcare_sentence(
        ds, domain="restaurant", either=either, is_one_sentence=is_one_sentence, wo_para=wo_para
    )
    res = first_sentence + second_sentence + "."
...
def attraction_state_to_sum(ds: Dict, either: callable, is_one_sentence: bool, idx: int, wo_para: bool) -> str:
    ...
    """
    first_sentence = get_first_sentence(
        ds, domain="attraction", either=either, except_keys=set(), idx=idx, wo_para=wo_para
    )

    second_sentence = get_dontcare_sentence(
        ds, domain="attraction", either=either, is_one_sentence=is_one_sentence, wo_para=wo_para
    )
    res = first_sentence + second_sentence + "."
...
def hotel_state_to_sum(ds: Dict, either: callable, is_one_sentence: bool, idx: int, wo_para: bool) -> str:
    first_sentence = get_first_sentence(
        ds=ds, domain="hotel", either=either, except_keys={"hotel-parking", "hotel-internet"}, idx=idx, wo_para=wo_para
    )

    second_sentence = get_dontcare_sentence(
        ds, domain="hotel", either=either, is_one_sentence=is_one_sentence, wo_para=wo_para
    )
...
def train_sum_to_state(summ: str, is_one_sentence: bool) -> dict:
    ...
    for slot, prefix in slot_to_prefix.items():
        ...
        if match:
            ...
            value = re.split(" The | Also, | which | for | from | to | on | and | people| person", summary[start_idx:])[
                0
            ]
...
def taxi_sum_to_state(summ: str, is_one_sentence: bool) -> Dict:
    ...
    for slot, prefix in slot_to_prefix.items():
        ...
        if match:
            ...
            value = re.split(" The | Also, | which | from | to | and ", summary[start_idx:])[0]
...
def attraction_sum_to_state(summ: str, is_one_sentence: bool) -> Dict:
    ...
    for slot, prefix in slot_to_prefix.items():
        ...
        if match:
            ...
            value = re.split(" The | Also, | which | called | located in the ", _summary,)[0]
...
class HeuristicConverter:
    ...
    def state_to_sum(self, ds: Dict, is_for_template: Optional[bool] = False, blank: Optional[str] = None,) -> str:
        ...
        sentences = [
            self.domain_state_to_sum[domain](
                ds, either=either, is_one_sentence=self.do_concat, idx=idx, wo_para=self.wo_para
            )
            for idx, domain in enumerate(appearing_domains)
        ]
        summary = " Also, ".join(
            sentences
        )  # when list is length 1, join does not add the Also, and just returns an str
...
def get_converter(converter_name: str):
    ...
    if converter_name == "wo_para":
        return HeuristicConverter(wo_para=True, do_concat=True)
    # without one sentence concatenating
    if converter_name == "wo_concat":
        return HeuristicConverter(wo_para=False, do_concat=False)
    if converter_name == "open_domain":
        return DomainFreeConverter()
    if converter_name == "vanilla":
        return HeuristicConverter(wo_para=True, do_concat=False)

    return HeuristicConverter(wo_para=False, do_concat=True)
...
if __name__ == "__main__":
    sample_states = [
        ...
        (
            "restaurant",
            {"restaurant-name": "meze bar", "restaurant-pricerange": "expensive", "restaurant-food": "dontcare",},
        ),
        (
            "restaurant",
            {"restaurant-name": "meze bar", "restaurant-pricerange": "expensive", "restaurant-food": "seafood",},
        ),
        (
            "train",
            {
        ...
        (
            "attraction",
            {"attraction-area": "cambridge", "attraction-name": "nusha", "attraction-type": "entertainment",},
        ),
        (
    ...
    for conv in ["mwz", "wo_para", "wo_concat", "open_domain"]:
        for _domain, _state in sample_states:
            ...
            print(
                f'{Bcolors.HEADER}Test {f"{Bcolors.OKGREEN}passed{Bcolors.ENDC}" if _state == encoded else f"{Bcolors.FAIL}failed{Bcolors.ENDC}"}!'
            )
