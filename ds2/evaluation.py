...
def get_acc(
    ...
    for _pred, _correct, _eval_slots, _domains in zip(
        pred_slot_values, correct_slot_values, eval_slots, domains_per_sample
    ):
        res["jga"] += set((k, v) for k, v in _pred.items() if k in _eval_slots) == set(
        ...
        res[f"slot_true_acc"] += sum((k in _pred and k in _correct and _pred[k] == _correct[k]) for k in _eval_slots)
        res[f"slot_none_acc"] += sum((k not in _pred and k not in _correct) for k in _eval_slots)
        ...
        for _domain in _domains:
            ...
            res[f"{_domain}_jga"] += set((k, v) for k, v in _pred.items() if k in domain_eval_slots) == set(
            ...
            res[f"{_domain}_slot_true_acc"] += sum(
                (k in _pred and k in _correct and _pred[k] == _correct[k]) for k in domain_eval_slots
            )
            res[f"{_domain}_slot_none_acc"] += sum((k not in _pred and k not in _correct) for k in domain_eval_slots)
    ...
    def safe_devision(x: float, y: int):
        return round(x / y, ndigits=4) if y > 0 else 0
...
def get_template_acc(
    pred_summary: List[str], gold_templates: List[str], blank: str = "____",
):
