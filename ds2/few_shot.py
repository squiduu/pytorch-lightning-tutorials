...
EXCEPT_VALUES = {
    ...
    "train-day": ["fr", "n", "we", "train"],
    "train-departure": ["a",],
    "train-arrive by": ["thursday", "1100", "tuesday",],
    "train-book people": ["`1",],
    "train-leave at": [
        "after 9:15 am",
        "bishops stortford",
    ...
    "restaurant-food": ["kor", "steakhouse",],
    "restaurant-area": ["centre", "south", "north", "east", "west"],
    "attraction-name": ["no", "ay", "fi", "free", "bo", "by", "s", "c", "ac", "k", "mi"],
    "restaurant-name": ["no", "golden house                            golden house", "au", "el", "yes", "d", "hu",],
    "hotel-name": ["to", "n", "a", "hotel", "yes", "me", "nus", "gra", "the", "ca"],
    "taxi-leave at": ["after 2:30", "1615", "after 11:45", "thursday", "after 15:45", "300", "monday", "friday", "1"],
    "taxi-destination": ["shiraz.",],
    "taxi-departure": ["w", "g", "ca", "07:15",],
    "restaurant-book time": ["1545", "1745", "not given",],
    "restaurant-book day": ["thur", "not given", "w"],
    "taxi-arrive by": ["1145", "1730", "1700",],
}
...
def get_balanced_fewshot_samples(final_states: dict, num_target_samples: int, ALL_SLOTS) -> dict:
    ...
    domains = {dial_id: tuple(sorted(set(_slot.split("-")[0] for _slot in ds))) for dial_id, ds in final_states.items()}
...
def has_except_value(ds):
    return any(ds[_key] in EXCEPT_VALUES[_key] for _key in set(ds.keys()) & set(EXCEPT_VALUES.keys()))
